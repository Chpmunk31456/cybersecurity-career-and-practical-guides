# How to Build a Zero Trust Security Program

- [Español — DOCX](./Espanol/Como_Crear_un_Programa_Seguridad_Confianza_Cero_Espanol_v1.0.docx)
- [Español — MD](./Espanol/Como_Crear_un_Programa_Seguridad_Confianza_Cero_Espanol_v1.0.md)
- [Español — PDF](./Espanol/Como_Crear_un_Programa_Seguridad_Confianza_Cero_Espanol_v1.0.pdf)

## Cómo crear un programa de seguridad de Confianza Cero

A practical bilingual guide for planning, implementing, operating, measuring, and improving a Zero Trust security program.

## English Files

- [English Word Edition](./English/How_to_Build_a_Zero_Trust_Security_Program_English_v1.0.docx)
- [English PDF Edition](./English/How_to_Build_a_Zero_Trust_Security_Program_English_v1.0.pdf)
- [English Markdown Edition](./English/How_to_Build_a_Zero_Trust_Security_Program_English_v1.0.md)

## Archivos en español

## Topics Covered

- Zero Trust principles
- Identity and access management
- Device security
- Network segmentation
- Applications and workloads
- Data protection
- Visibility and analytics
- Automation and orchestration
- Privileged access
- Third-party access
- Cloud security
- OT and ICS considerations
- Privacy and accessibility
- Maturity assessment
- Testing and validation
- Metrics and executive reporting
- 30-, 60-, and 90-day implementation plan

## Intended Audience

- Students
- Career changers
- Cybersecurity professionals
- Identity and access professionals
- Cloud security engineers
- Security architects
- GRC professionals
- Security managers
- Consultants
- Technology leaders
- Small organizations and nonprofits

## Author

**Alberto (Al) Leiva**

## Version

Version 1.0 — July 2026

## License

Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International.

## Disclaimer

This manual is provided for education and general cybersecurity guidance. Zero Trust controls should be adapted to the organization’s systems, users, data, risks, legal obligations, operational needs, and available resources.

Security changes must be tested and authorized before deployment in production environments.

[Return to the main repository](../../README.md)
